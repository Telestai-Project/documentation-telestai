# Lasko Backend System Documentation

## Overview

Lasko's backend is designed to be decentralized, allowing community members to run their own nodes. In this early **testnet version**, we are testing several key components to ensure they work smoothly under load. This guide provides insights into how the backend operates and how each module functions. 
...
Cassandra is a **distributed NoSQL database** known for its high availability, horizontal scalability, and fault tolerance. In the Lasko backend, **Cassandra** stores user profiles, posts, and votes, with **data replicated across nodes** to ensure reliability.

### Key Features of Cassandra:
- **Decentralization**: Perfect for systems like Lasko where multiple nodes store and serve data independently.
...
Follow these steps to set up and run the Lasko backend on your own node:

Note: You'll need to have Docker installed to run Lasko Backend.

### 1. **Clone the Repository**
```bash
git clone https://github.com/Telestai-Project/Lasko-backend.git
cd Lasko-backend
```
...
This version of Lasko is an **early testnet release**, designed to ensure that the backend system is functional and stable. The combination of **FastAPI with Uvicorn** and **Cassandra as the database** provides the scalability needed for a distributed system. 
