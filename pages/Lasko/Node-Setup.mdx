# **Lasko Node Setup Guide (v3)**

This guide provides detailed instructions for setting up a **Lasko v3 Node**. Running a node helps decentralize the Lasko platform and participate in the testnet. Nodes host a **database and API** and are designed to be lightweight, independent, and easy to set up. 
...
```bash
sudo nano /etc/nginx/sites-available/Lasko-node.conf
```
...
```bash
sudo ln -s /etc/nginx/sites-available/Lasko-node.conf /etc/nginx/sites-enabled/
sudo nginx -t
...
To get your node listed on the [Lasko platform](https://Lasko.telestai.io):
...
2. Submit a pull request to the [Lasko Nodes Repository](https://github.com/Telestai-Project/Lasko-nodes) with the following format:
...
Once approved, your node will appear on the Lasko frontend.
...
```bash
sudo nano /etc/systemd/system/Lasko-node.service
```
...
```bash
sudo systemctl enable Lasko-node.service
sudo systemctl start Lasko-node.service
```
